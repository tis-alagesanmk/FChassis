...
public class JSONFileWrite : FileWrite {
         ...
         case TypeCode.Object:
            if (dataType.IsEnum)
               ...
               if (_isUserDefinedClassArray (dataType))
                  return;
               if(!dataType.GetElementType()!.IsClass)
   ...
   void writeArray (Utf8JsonWriter writer, string name, Array array) {
      ...
      // Lambda to handle writing each element
      Action<object> writeElement = element =>
      {
         switch (Type.GetTypeCode (elementType)) {
            case TypeCode.String:
               ...
               break;
            case TypeCode.Int32:
               writer.WriteNumberValue ((int)element);
               break;
            case TypeCode.Double:
      ...
      // Use the lambda to write each element
